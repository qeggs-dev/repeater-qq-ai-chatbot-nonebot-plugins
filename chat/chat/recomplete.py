...
async def handle_recomplete(bot: Bot, event: MessageEvent, args: Message = CommandArg()):
    ...
    if RepeaterDebugMode:
        ...
        filename, code, text, reasoning, content = await chat_core.send_message_and_get_image(message=msg, username=nickname, recomplete=True, model_type='chat')
        if mode == "group":
            if code == 200:
                response = MessageSegment.image(filename)
                await recomplete.finish(reply + response)
            else:
            ...
            if code == 200:
                if len(text) > MAX_LENGTH:
                    ...
                    response = text
                await recomplete.finish(reply + content)
            else:
                await recomplete.finish(reply + f'====Chat.Recomplete====\n> {session_id}\n{response}\nHTTP Code: {code}')
