...
async def handle_npchat(bot: Bot, event: MessageEvent, args: Message = CommandArg()):
    ...
    if msg:
        ...
        if RepeaterDebugMode:
            await npchat.finish(reply + f'[Chat.No_Prompt_Chat|{session_id}|{nickname}]：{msg}')
        else:
            response = await chat_core.send_message(message=msg, username=nickname, load_prompt=False)
            ...
            if response['status_code'] == 200:
                ...
                await npchat.finish(reply + message)
            else:
                await npchat.finish(reply + f"====Chat.No_Prompt_Chat====\n> {session_id}\n{response}\nHTTP Code: {response['status_code']}")
