...
async def handle_reference(bot: Bot, event: MessageEvent, args: Message = CommandArg()):
    ...
    response = await chat_core.send_message(message=stranger_info.message.strip(), username=stranger_info.nickname, reference_context_id=stranger_info.noself_at_list[0])
