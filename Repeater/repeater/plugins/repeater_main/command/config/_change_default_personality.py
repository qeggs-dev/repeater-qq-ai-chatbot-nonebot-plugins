...
async def handle_change_default_personality(bot: Bot, event: MessageEvent, args: Message = CommandArg()):
    ...
    sendmsg = SendMsg("Config.Change_Default_Personality", change_default_personality, stranger_info)
    
    config_core = ConfigCore(stranger_info)
    if sendmsg.is_debug_mode:
        await sendmsg.send_debug_mode()
    else:
        response = await config_core.set_config("parset_prompt_name", stranger_info.message_str)
